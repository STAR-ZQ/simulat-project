...
public class RandomNumber {
     ...
     * @return 随机结果
     */
    public static Integer produceRandomNumber(Integer min, Integer max) {
    ...
    public static double produceRateRandomNumber(double min, double max, List<Double> separates, List<Double> percents) {
        ...
        for (int i = 0; i < rangeCount; i++) {
            ...
            range.percentScopeMax = (int) (range.percentScopeMin + (range.percent - 1));
     ...
     * @return 按比率随机结果
     */
    public static Integer produceRateRandomNumber(Integer min, Integer max, List<Integer> separates, List<Double> percents) {
        if (min > max) {
            throw new IllegalArgumentException("min值必须小于max值");
        }
        if (separates == null || percents == null || separates.size() == 0) {
        ...
        for (int i = 0; i < rangeCount; i++) {
            Range range = new Range();
            range.minInteger = (i == 0 ? min : separates.get(i - 1));
            range.maxInteger = (i == rangeCount - 1 ? max : separates.get(i));
            range.percent = percents.get(i);
        ...
        for (int i = 0; i < ranges.size(); i++) {
            ...
            //判断使用哪个range产生最终的随机数
            if (range.percentScopeMin <= randomInt && randomInt <= range.percentScopeMax) {
                r = produceRandomNumber(range.minInteger, range.maxInteger);
                break;
            }
        }
        return r;
